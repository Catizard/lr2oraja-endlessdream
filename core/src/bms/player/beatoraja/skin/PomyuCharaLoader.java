    ...
    public SkinImage load(boolean usecim, File imagefile, int type, int color, float dstx, float dsty, float dstw, float dsth, int side, int dsttimer, int dstOp1, int dstOp2, int dstOp3, int dstOffset) {
        ...
        try {
            ...
            if (imagefile.exists() && imagefile.getPath().substring(imagefile.getPath().length() - 4, imagefile.getPath().length()).equalsIgnoreCase(".chp")) {
                ...
                if (imagefile.getPath().charAt(imagefile.getPath().length() - 1) != '/' && imagefile.getPath().charAt(imagefile.getPath().length() - 1) != '\\')
                    ...
                    if (line.startsWith("#")) {
                        ...
                        if (str.length > 1) {
                            ...
                            if (str[0].equalsIgnoreCase("#CharBMP")) {
                                //#Pattern, #Layer用画像
                                if (data.size() > 1)
                                    CharBMP[CharBMPIndex] = SkinLoader.getTexture(chp.getPath().substring(0, Math.max(chp.getPath().lastIndexOf('\\'), chp.getPath().lastIndexOf('/')) + 1) + data.get(1).replace("\\", "/"), usecim);
                            } else if (str[0].equalsIgnoreCase("#CharBMP2P")) {
                                //#Pattern, #Layer用画像2P
                                if (data.size() > 1)
                                    CharBMP[CharBMPIndex + 1] = SkinLoader.getTexture(chp.getPath().substring(0, Math.max(chp.getPath().lastIndexOf('\\'), chp.getPath().lastIndexOf('/')) + 1) + data.get(1).replace("\\", "/"), usecim);
                            } else if (str[0].equalsIgnoreCase("#CharTex")) {
                                //#Texture用画像
                                if (data.size() > 1)
                                    CharBMP[CharTexIndex] = SkinLoader.getTexture(chp.getPath().substring(0, Math.max(chp.getPath().lastIndexOf('\\'), chp.getPath().lastIndexOf('/')) + 1) + data.get(1).replace("\\", "/"), usecim);
                            } else if (str[0].equalsIgnoreCase("#CharTex2P")) {
                                //#Texture用画像2P
                                if (data.size() > 1)
                                    CharBMP[CharTexIndex + 1] = SkinLoader.getTexture(chp.getPath().substring(0, Math.max(chp.getPath().lastIndexOf('\\'), chp.getPath().lastIndexOf('/')) + 1) + data.get(1).replace("\\", "/"), usecim);
                            } else if (str[0].equalsIgnoreCase("#CharFace")) {
                                //ハリアイ
                                if (data.size() > 1)
                                    CharBMP[CharFaceIndex] = SkinLoader.getTexture(chp.getPath().substring(0, Math.max(chp.getPath().lastIndexOf('\\'), chp.getPath().lastIndexOf('/')) + 1) + data.get(1).replace("\\", "/"), usecim);
                            } else if (str[0].equalsIgnoreCase("#CharFace2P")) {
                                //ハリアイ2P
                                if (data.size() > 1)
                                    CharBMP[CharFaceIndex + 1] = SkinLoader.getTexture(chp.getPath().substring(0, Math.max(chp.getPath().lastIndexOf('\\'), chp.getPath().lastIndexOf('/')) + 1) + data.get(1).replace("\\", "/"), usecim);
                            } else if (str[0].equalsIgnoreCase("#SelectCG")) {
                                //選択画面アイコン
                                if (data.size() > 1)
                                    CharBMP[SelectCGIndex] = SkinLoader.getTexture(chp.getPath().substring(0, Math.max(chp.getPath().lastIndexOf('\\'), chp.getPath().lastIndexOf('/')) + 1) + data.get(1).replace("\\", "/"), usecim);
                            } else if (str[0].equalsIgnoreCase("#SelectCG2P")) {
                                //選択画面アイコン2P
                                if (data.size() > 1)
                                    CharBMP[SelectCGIndex + 1] = SkinLoader.getTexture(chp.getPath().substring(0, Math.max(chp.getPath().lastIndexOf('\\'), chp.getPath().lastIndexOf('/')) + 1) + data.get(1).replace("\\", "/"), usecim);
                                ...
                                if (data.size() > 2) {
                                    if (PMparseInt(data.get(1)) >= 0 && PMparseInt(data.get(1)) < frame.length)
                                        frame[PMparseInt(data.get(1))] = PMparseInt(data.get(2));
                                }
                                ...
                                //ループ位置
                                if (data.size() > 2) {
                                    if (PMparseInt(data.get(1)) >= 0 && PMparseInt(data.get(1)) < loop.length)
                                        loop[PMparseInt(data.get(1))] = PMparseInt(data.get(2));
                                }
                            }
                        }
                    }
                                ...
                                if (timer != Integer.MIN_VALUE
                                        ...
                                        if (dst[1].length() >= i + 2) {
                                            if (dst[1].substring(i, i + 2).equals("--")) {
                                                ...
                                                for (int j = i; j < dst[1].length() && dst[1].substring(j, j + 2).equals("--"); j += 2)
                                                    count++;
                                                if (PMparseInt(dst[1].substring(i + count * 2, i + count * 2 + 2), 36) >= 0 && PMparseInt(dst[1].substring(i + count * 2, i + count * 2 + 2), 36) < xywh.length)
                                                    endxywh = xywh[PMparseInt(dst[1].substring(i + count * 2, i + count * 2 + 2), 36)];
                                                for (int j = i; j < dst[1].length() && dst[1].substring(j, j + 2).equals("--"); j += 2) {
                                    ...
                                    if ((loopFrame + increaseRate) != 0) {
                                        ...
                                        for (int i = 0; i < (loop[motion] + 1) * 2; i += 2) {
                                            ...
                                            if (index >= 0 && index < xywh.length && xywh[index][2] > 0 && xywh[index][3] > 0)
                                                images[i / 2] = new TextureRegion(setBMP, xywh[index][0], xywh[index][1], xywh[index][2], xywh[index][3]);
                                    ...
                                    for (int i = (loop[motion] + 1) * 2; i < dst[0].length(); i += 2) {
                                        int index = PMparseInt(dst[0].substring(i, i + 2), 36);
                                        if (index >= 0 && index < xywh.length && xywh[index][2] > 0 && xywh[index][3] > 0)
                                            images[i / 2 - (loop[motion] + 1)] = new TextureRegion(setBMP, xywh[index][0], xywh[index][1], xywh[index][2], xywh[index][3]);
                                        else
                                            images[i / 2 - (loop[motion] + 1)] = new TextureRegion(transparent, 0, 0, 1, 1);
                                    }
