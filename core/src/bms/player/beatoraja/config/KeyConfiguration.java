	...
	public void render() {
		...
		for (int i = scrollpos; i < keys.length; i++) {
			...
			if (i == cursorpos) {
				...
				shape.rectangle(200 * scaleX, y * scaleY, 80 * scaleX, 24 * scaleY);
				shape.rectangle(350 * scaleX, y * scaleY, 80 * scaleX, 24 * scaleY);
				shape.rectangle(650 * scaleX, y * scaleY, 80 * scaleX, 24 * scaleY);
				sprite.end();
			}
			sprite.begin();
			if(titlefont != null) {
