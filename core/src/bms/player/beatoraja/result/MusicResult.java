...
import static bms.player.beatoraja.modmenu.FreqTrainerMenu.isFreqNegative;
        ...
        if (resource.getCourseBMSModels() != null) {
            ...
            if (resource.getGauge()[resource.getGrooveGauge().getType()].get(resource.getGauge()[resource.getGrooveGauge().getType()].size - 1) > 0) {
                if (resource.getAssist() > 0) {
                    if (resource.getAssist() == 1 && cscore.getClear() != ClearType.AssistEasy.id)
                        cscore.setClear(ClearType.LightAssistEasy.id);
