        ...
        if (flag) {
            ...
            if (candidate.size() != 0) {
                ...
                List<Integer> rendaLane = lastNoteTime.keySet().stream()
                        .filter(lane -> {
                            return tl.getTime() - lastNoteTime.get(lane) < threshold;
                        })
                        .collect(Collectors.toList());
                List<List<Integer>> candidate2 = candidate.stream()
                        .map(lanes -> {
                            return lanes.stream()
                                    .filter(lane -> {
                                        return !rendaLane.contains(lane);
                                    })
                                    .collect(Collectors.toList());
                        })
                        .filter(lanes -> {
                            return lanes.size() >= noteCount;
                        })
                        .collect(Collectors.toList());
